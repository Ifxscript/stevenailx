/**
 * SERVICES SYNC — Pushes the complete services catalog to Firestore.
 * Run once: node scripts/sync_services.mjs
 * Writes each category (nails, salon) as a document in the 'services' collection.
 */
...
const salonServices = {
  ...
  services: [
    ...
    },
    {
      ...
      sections: [
        {
          title: "Braids (Extensions Included)",
          items: [
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            { id: "s_lh4", name: "Lash Refill", price_from: 0, price_note: "50% of original price" }
          ]
        }
      ]
    }
...
async function syncServices() {
  console.log("🚀 Syncing complete services catalog to Firestore...\n");

  await setDoc(doc(db, "services", "nails"), nailServices);
  console.log("  ✅ services/nails — 9 categories, 34 items");

  await setDoc(doc(db, "services", "salon"), salonServices);
  console.log("  ✅ services/salon — 3 categories (Hair, Braids, Lashes), 30 items");

  console.log("\n🎉 ALL SERVICES SYNCED! Total: 64 bookable items.");
  process.exit(0);
}

syncServices().catch(e => { console.error("❌ Sync failed:", e); process.exit(1); });
